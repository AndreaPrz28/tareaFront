  ...
  try {
    ...
    if (res.ok) {
      ...
      if (cuenta) {
        ...
        // Guardamos en el almacenamiento local la información de la cuenta del usuario que recibimos del servidor
        localStorage.setItem("cuenta", JSON.stringify(data.usuario.cuenta)); // Convertimos a texto usando JSON.stringify
        // Guardamos la contraseña que ingresó el usuario, ya que el backend no devuelve la contraseña
        // Como el usuario ya fue validado, podemos asumir que la contraseña es correcta
